...
class SendgridMailer(object):
    def __init__(self, subject, html_content, attachment_list=None):
        ...
        self.to_email_string_stw = os.getenv('TO_EMAILS_STW')
    ...
    def parse_to_emails(self):
        ...
        self.to_emails_stw = []
        names = self.to_email_string_stw.split(",")
        for name in names:
            item = name.split("|")
            item_tuple = (item[0], item[1] + " " + item[2])
            # self.to_emails_stw.append(item_tuple)
            self.to_emails_stw.append(item[0])
